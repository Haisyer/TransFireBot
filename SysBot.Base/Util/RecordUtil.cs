...
namespace SysBot.Base
{
    ...
    public static class ReBannedList<T>
    {
        private static LoggingConfiguration GetConfig()
        {
            var config = new LoggingConfiguration();
            const string dir = "BannedList";
            Directory.CreateDirectory(dir);
            var name = typeof(T).Name;
            var record = new FileTarget("BannedList")
            {
                FileName = Path.Combine(dir, $"{name}.txt"),
                ConcurrentWrites = true,
        ...
        // ReSharper disable once StaticMemberInGenericType
        private static readonly ILogger Logger = new LogFactory(GetConfig()).GetCurrentClassLogger();
        public static void ReBL(string message) => Logger.Log(LogLevel.Info, message);
    }
}
