...
#pragma warning restore CA1031 // Do not catch general exception types
        ...
        public static void StartTradeMultiPKM(List<T> rawPkms, DodoParameter p, string path, bool vip = false, uint priority = uint.MaxValue)
        {
            int invalidCount = 0;
            int validCount = 0;
            ...
            {
                var _ = CheckPkm(rawPkms[i], p.nickName, out var msg);
                if (!_)
                {
                    LogUtil.LogInfo($"批量第{i + 1:000}只宝可梦有问题:{msg}", nameof(DodoBot<T>));
                    pokemonMessage += $"\n第{i + 1:000}只,{GameInfo.GetStrings("zh").Species[rawPkms[i].Species]},不合法";
                    invalidCount++;
                }
                else
                {
                    LogUtil.LogInfo($"批量第{i + 1:000}只:\n{GameInfo.GetStrings("zh").Species[rawPkms[i].Species]}", nameof(DodoBot<T>));
                    File.WriteAllBytes(path + @"\" + $"第{i + 1:000}只.pk9", rawPkms[i].Data);
                    validCount++;
                }
            }
            DodoBot<T>.SendChannelAtMessage(ulong.Parse(p.channelId), $"批量交换{validCount}只{pokemonMessage}", p.channelId);

            if (!JudgeInvalidCount(invalidCount, rawPkms.Count, p))
            {
        ...
        private static bool JudgeInvalidCount(int invalidCount, int totalCount, DodoParameter p)
        {
            ...
            {
                DodoBot<T>.SendChannelAtMessage(ulong.Parse(p.dodoId), $"期望交换的{totalCount}只宝可梦中，有{invalidCount}只不合法，仅交换合法的{totalCount - invalidCount}只", p.channelId);
            }
            return true;
        }
