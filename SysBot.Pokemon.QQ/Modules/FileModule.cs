...
namespace SysBot.Pokemon.QQ
{
    public class FileModule<T> : IModule where T : PKM, new()
    {
        ...
        public async void Execute(MessageReceiverBase @base)
        {
            ...
            if (!FileTradeHelper<T>.IsValidFileName(fileName) || !FileTradeHelper<T>.IsValidFileSize(fileMessage.Size))
            {
            ...
            if (pkms.Count > 1 && pkms.Count <= FileTradeHelper<T>.maxPokemonCountInBin)
                new MiraiQQHelper<T>(senderQQ, nickname).StartTradeMultiPKM(pkms,senderQQ);
            else if (pkms.Count == 1)
                new MiraiQQHelper<T>(senderQQ, nickname).StartTradePKM(pkms[0]);
            else
